...
public class AuthService {
    ...
    public AuthResponse login(AuthRequest request) {
        ...
        // --- NEW: Check if the user account is still pending admin approval ---
        if (user.getStatus() != null && user.getStatus().name().equals("PENDING")) {
            throw new IllegalArgumentException("Your account is pending admin approval. Please wait for activation.");
        }

        // --- NEW: Check if the user account is blocked ---
        if (user.isBlocked()) {
            throw new IllegalArgumentException("Your account has been blocked. Reason: " + user.getBlockReason());
        }
