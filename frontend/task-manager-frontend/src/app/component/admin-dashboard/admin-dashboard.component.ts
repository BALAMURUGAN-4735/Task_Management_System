...
@Component({
  ...
  imports: [
    ...
    MatSidenavModule, MatListModule, MatProgressBarModule, MatMenuModule, NavbarComponent, MatFormFieldModule, MatDatepickerModule,
...
export class AdminDashboardComponent implements OnInit {
 ...
 allocatedTeamsList: any[] = [
  ...
  this.webSocketService.taskUpdates$.subscribe(() => {
    console.log('🔄 Admin Dashboard: Task update received');
    this.loadDashboardData();
  });
}
  ...
  let logsToExport = [...this.filteredAuditLogs];
  ...
  if (this.exportFromDate || this.exportToDate) {
    logsToExport = logsToExport.filter(log => {
      ...
      if (this.exportFromDate) {
        const fromDate = new Date(this.exportFromDate);
        fromDate.setHours(0, 0, 0, 0);
        valid = valid && logTime >= fromDate.getTime();
      }

      if (this.exportToDate) {
        const toDate = new Date(this.exportToDate);
        toDate.setHours(23, 59, 59, 999);
        valid = valid && logTime <= toDate.getTime();
      }
  ...
  if (logsToExport.length === 0) {
    alert('No audit logs found for the selected filters/date range.');
    return;
  }
  ...
  // =========================
    ...
    const rows = logsToExport.map(log => [
      log.action || '',
      log.performedBy || log.performed_by || 'UNKNOWN_USER',
      log.details || '',
