...
export class EmployeeDashboardComponent implements OnInit {
  ...
  this.webSocketService.taskUpdates$.subscribe(() => {
    console.log('🔄 Employee Dashboard: Task update received');
    this.loadEmployeeTasks();
  });
}
...
loadEmployeeTasks(): void {
  ...
  this.taskService.getTasksForEmployee(this.currentUserId).subscribe({
    next: (res: any) => {
      ...
      console.log('📋 Tasks for this employee:', rawTasks);

      this.allAssignedTasks = rawTasks.map((t: any) => ({
        ...t,
    ...
    error: (err) => {
      console.error('❌ Employee task API error:', err);
      this.allAssignedTasks = [];
      this.filteredTasks = [];
      this.calculateKPIs();
      this.calculateWeeklyCapacity();
